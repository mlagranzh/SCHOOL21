...
int	key_press(int keycode, t_vars *vars)
{
	...
	if (keycode == 20)
		vars -> color = -100;
	if (!ft_strncmp(vars -> fract_ol, "mandelbar", 10))
		mandelbar(vars);
	if (!ft_strncmp(vars -> fract_ol, "mandelbrot", 11))
		mandelbrot(vars);
	if (!ft_strncmp(vars -> fract_ol, "julia", 6))
		julia(vars);
	return (0);
}
...
int	main(int argc, char **argv)
{
	...
	else if (!ft_strncmp(argv[1], "mandelbar", 10))
		mandelbar(&vars);
	else
	{
		printf("Укажи название фрактала: mandelbrot/julia/mandelbar\n");
